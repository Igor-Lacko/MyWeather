...
class StackedGraphLayout(QStackedLayout):
    ...
    def __init__(self, data : Day, location : str, color_mode : ColorModes):
        ...
        self.temperature_graph = TemperatureGraph(color_mode, [hour.temperature_data.actual_temperature for hour in data.hours],
        [hour.temperature_data.feelslike for hour in data.hours])
        self.wind_graph = WindSpeedGraph(color_mode, [hour.wind_data.speed for hour in data.hours])
        self.rain_graph = RainGraph(color_mode, [hour.humidity for hour in data.hours], 
        [hour.rain_data.rain_chance for hour in data.hours])
        self.precip_graph = PrecipitationGraph(color_mode, [hour.rain_data.precip_height for hour in data.hours])
